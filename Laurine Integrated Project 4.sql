...
-- Creating a project progress report that shows thw locations, source type, imprvement, source status and date of completion
...
-- Well Improvement. Depending on whether they are chemically contaminated = RO, or biologically contaminated = Uv & RO
SELECT
location.address,
location.town_name,
location.province_name,
...
well_pollution.results,
CASE
    WHEN well_pollution.results = 'Contaminated: Biological' THEN 'Install UV & RO'
    WHEN well_pollution.results = 'Contaminated: Chemical' THEN 'Install RO'
    ELSE NULL
  END AS Improvement
FROM
water_source
